...
function emitRust(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    server.push(`use super::${typesMod}::{${svc.models.filter(m => m.name).map(m => m.name).join(", ")}};\n`);
    ...
    const clientImports = [constPrefix.toUpperCase(), ...svc.models.filter(m => m.name).map(m => m.name)].join(", ");
    client.push(`use super::${typesMod}::{${clientImports}};\n`);
    ...
    for (const rpc of svc.rpcs) {
      if (rpc.isStream) {
        ...
        client.push(`        self.inner.stream(${constPrefix}_${rpc.originalName.toUpperCase()}_PROCEDURE, req).await`);
      } else {
        client.push(`    pub async fn ${rpc.name}(&self, req: &${reqName(rpc)}) -> Result<${resName(rpc)}, SpeconnError> {`);
        client.push(`        self.inner.call(${constPrefix}_${rpc.originalName.toUpperCase()}_PROCEDURE, req).await`);
      }
      client.push(`    }`);
    }
    client.push('}\n');
