...
function emitRust(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    server.push(`use speconn::{SpeconnRouter, SpeconnError};`);
    server.push(`use super::${typesMod}::*;\n`);
    server.push(`pub fn ${camelBase(svc.serviceName)}_router(`);
    for (const rpc of svc.rpcs) {
      server.push(`    ${rpc.name}_fn: impl Fn(${reqName(rpc)}) -> Result<${resName(rpc)}, SpeconnError> + Send + Sync + 'static,`);
    }
    ...
    client.push(`use speconn::{SpeconnClient as SpeconnClientBase, SpeconnError};`);
    client.push(`use super::${typesMod}::*;\n`);
    client.push(`pub struct ${svc.serviceName}Client {`);
    client.push(`    inner: SpeconnClientBase,`);
    client.push('}\n');
    client.push(`impl ${svc.serviceName}Client {`);
    client.push(`    pub fn new(base_url: &str) -> Self { ${svc.serviceName}Client { inner: SpeconnClientBase::new(base_url) } }`);
    for (const rpc of svc.rpcs) {
      if (rpc.isStream) {
        client.push(`    pub async fn ${rpc.name}(&self, req: &${reqName(rpc)}) -> Result<Vec<${resName(rpc)}>, SpeconnError> {`);
        client.push(`        self.inner.call_server_stream(${constPrefix}_${rpc.originalName.toUpperCase()}_PROCEDURE, req).await`);
      } else {
        client.push(`    pub async fn ${rpc.name}(&self, req: &${reqName(rpc)}) -> Result<${resName(rpc)}, SpeconnError> {`);
        client.push(`        self.inner.call_unary(${constPrefix}_${rpc.originalName.toUpperCase()}_PROCEDURE, req).await`);
      }
      client.push(`    }`);
    }
    client.push('}\n');
